...
class BLEManagerEsp: NSObject {
    static let instance = BLEManagerEsp()
    ...
    func sendData(data: Data, callback: @escaping (String?) -> ()) {
        ...
        for periph in readyPeripherals {
            if let char = BLEManagerEsp.instance.getCharForUUID(writeCBUUID, forperipheral: periph) {
    ...
    func sendStopCityData(data: Data, callback: @escaping (String?) -> ()) {
        sendDataCallback = callback
        for periph in readyPeripherals {
            if let char = BLEManagerEsp.instance.getCharForUUID(readCityCBUUID, forperipheral: periph) {
                periph.writeValue(data, for: char, type: CBCharacteristicWriteType.withResponse)
            }
        }
    }
    ...
    func sendStopData(data: Data, callback: @escaping (String?) -> ()) {
        sendDataCallback = callback
        for periph in readyPeripherals {
            if let char = BLEManagerEsp.instance.getCharForUUID(readCBUUID, forperipheral: periph) {
                periph.writeValue(data, for: char, type: CBCharacteristicWriteType.withResponse)
            }
        }
    }
...
extension BLEManagerEsp: CBPeripheralDelegate {
...
extension BLEManagerEsp: CBCentralManagerDelegate {
